...
function addMasterVolume() {
  ...
  section.className = 'slider master';
  id = 'master-volume';
  section.id = id;

  section.innerHTML = `
  <label for="${id}">Master Volume</label>
  <span role="img" aria-label="${slider.label}">${slider.emoji}</span>
  <input type="range" min="0" max="100" value="${slider.value}"  id="${slider.id}">
`;
...
function addSliders() {
  sliders.forEach(slider => {
    ...
    section.innerHTML = `
  	<label for="${slider.id}">${slider.label} Noises</label>
    <span role="img" aria-label="${slider.label}">${slider.emoji}</span>
    <input type="range" min="0" max="100" value="${slider.value}"  id="${slider.id}">
  `;
