  ...
  assignDirections(piece, currentDirections, i, j) {
    currentDirections.forEach(dir => {
      ...
      if (dir === 'top' && i > 0) {
        ...
        let aboveInnerDiv = abovePiece.querySelector(
          '.inner-piece.bottom-add, .inner-piece.bottom-remove'
        );
        isAdd = aboveInnerDiv && aboveInnerDiv.classList.contains('bottom-remove');
      }
      if (dir === 'left' && j > 0) {
        let leftPiece = document.querySelector(`.piece-${i}-${j - 1}`);
        let leftInnerDiv = leftPiece.querySelector(
          '.inner-piece.right-add, .inner-piece.right-remove'
        );
